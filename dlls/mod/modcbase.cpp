...
int ModDispatchSpawn( edict_t *pent )
{
	...
	if (pEntity)
	{
		...
		// swap the model name as spawn time - overriding the model name that gets passed to engine
		// functions works but the wrong model name in entvars_t still causes problems in the engine
		if ( strlen( szModelName ) && g_pFileSystem && !g_pFileSystem->FileExists( szModelName ) )
		{
