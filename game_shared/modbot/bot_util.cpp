...
bool UTIL_IsNameTaken( const char *name, bool ignoreHumans )
{
	for ( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		...
		if (player->IsPlayer() && (((CModPlayer *)player)->IsBot() == TRUE))
		{
...
int UTIL_HumansInGame( bool ignoreSpectators )
{
	...
	for (int iIndex = 1; iIndex <= gpGlobals->maxClients; iIndex++ )
	{
		...
		CModPlayer *player = static_cast<CModPlayer *>( entity );

		if (player->IsBot())
			continue;

		if (ignoreSpectators && player->pev->flags & FL_SPECTATOR)
			continue;
...
int UTIL_BotsInGame( void )
{
	...
	for (int iIndex = 1; iIndex <= gpGlobals->maxClients; iIndex++ )
	{
		CModPlayer *pPlayer = static_cast<CModPlayer *>(UTIL_PlayerByIndex( iIndex ));
